...
// Clone rows to reactive
const rows = ref([...props.tasks])
// Check if the props changes and sync
watch(()=> props.tasks,(value)=>{
    rows.value = [...value]
})

// Reorder config, I used IA to understand the using of this tool, calls for tasks.reorder method
const onReorder = () => {
    const payload = rows.value.map((t, i) => ({ id: t.id, priority: i + 1 }))
    router.post(route('tasks.reorder'), { tasks: payload }, {
        preserveScroll: true,
        replace: true, // no contaminar historial
    })
}
                            ...
                            <SingleTableHeader>
                                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col" class="px-4 py-3 text-right">Actions
                                            <span class="sr-only">Actions</span>
                                        </th>
                                        <th scope="col" class="px-4 py-3">Reorder</th>
                                    </tr>
                                    </thead>
                                    <draggable v-model="rows" item-key="id" tag="tbody" handle=".drag-handle" @end="onReorder" :component-data = "{class: ''}">
                                        <template #item="{ element:task }">
                                            <tr :key="task.id" class="border-b dark:border-gray-700">
